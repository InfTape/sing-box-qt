...
AppContext::AppContext() {
  ...
  m_proxyController    = std::make_unique<ProxyController>(
      m_kernelService.get(), m_subscriptionService.get(),
      m_configRepository.get(), m_settingsStore.get(),
      m_systemProxyGateway.get());
  m_proxyUiController = std::make_unique<ProxyUiController>(
      m_proxyController.get(), m_kernelService.get(), m_settingsStore.get(),
      m_adminActions.get());
...
ProxyService* AppContext::proxyService() const { return m_proxyService.get(); }
...
ThemeService* AppContext::themeService() const { return m_themeService.get(); }
SystemProxyGateway* AppContext::systemProxy() const {
  return m_systemProxyGateway.get();
}
AdminActions* AppContext::adminActions() const { return m_adminActions.get(); }
