...
CoreManagerServer::CoreManagerServer(QObject* parent)
  ...
  connect(m_kernel, &KernelRunner::statusChanged, this,
          &CoreManagerServer::onKernelStatusChanged);
  connect(m_kernel, &KernelRunner::outputReceived, this,
          &CoreManagerServer::onKernelOutput);
  connect(m_kernel, &KernelRunner::errorOutputReceived, this,
          &CoreManagerServer::onKernelErrorOutput);
  connect(m_kernel, &KernelRunner::errorOccurred, this,
...
bool CoreManagerServer::startListening(const QString& name, QString* error) {
  ...
  if (!m_server->listen(name)) {
    if (error) {
      *error =
          QString("Failed to listen on %1: %2").arg(name, m_server->errorString());
    }
    return false;
  }
...
QString CoreManagerServer::serverName() const { return m_serverName; }
...
void CoreManagerServer::onNewConnection() {
  ...
  connect(m_client, &QLocalSocket::readyRead, this,
          &CoreManagerServer::onReadyRead);
  connect(m_client, &QLocalSocket::disconnected, this,
...
void CoreManagerServer::onClientDisconnected() { m_client = nullptr; }
...
void CoreManagerServer::handleMessage(const QJsonObject& obj) {
  ...
  sendResponse(id, false, QJsonObject(),
               QString("Unknown method: %1").arg(method));
}

void CoreManagerServer::sendResponse(int id, bool ok,
