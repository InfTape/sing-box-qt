...
{
    bool ok = false;
    const QString name = QInputDialog::getText(this, tr("Add Rule Set"),
                                               tr("Rule set name"), QLineEdit::Normal,
                                               "default", &ok).trimmed();
...
void ManageRuleSetsDialog::onSetRename()
{
    ...
    bool ok = false;
    const QString name = QInputDialog::getText(this, tr("Rename Rule Set"),
...
void ManageRuleSetsDialog::onSetContextMenu(const QPoint &pos)
{
    ...
    if (!chosen) return;
    if (chosen == addAct) onSetAdd();
    else if (chosen == renameAct) onSetRename();
    else if (chosen == delAct) onSetDelete();
}
...
void ManageRuleSetsDialog::onRuleContextMenu(const QPoint &pos)
{
    ...
    if (!chosen) return;
    if (chosen == addAct) onRuleAdd();
    else if (chosen == delAct) onRuleDelete();
}
