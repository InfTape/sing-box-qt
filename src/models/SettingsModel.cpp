...
SettingsModel::Data SettingsModel::load() {
  ...
  d.systemProxyBypass = config.value("systemProxyBypass").toString(ConfigConstants::DEFAULT_SYSTEM_PROXY_BYPASS);
  d.tunMtu            = config.value("tunMtu").toInt(ConfigConstants::DEFAULT_TUN_MTU);
  d.tunStack          = config.value("tunStack").toString(ConfigConstants::DEFAULT_TUN_STACK);
  ...
  d.dnsProxy          = config.value("dnsProxy").toString(ConfigConstants::DEFAULT_DNS_PROXY);
