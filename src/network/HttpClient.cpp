...
void HttpClient::setAuthToken(const QString& token) { m_authToken = token; }
void HttpClient::setTimeout(int msecs) { m_timeout = msecs; }
...
void HttpClient::put(const QString& url, const QByteArray& data,
                     Callback callback) {
  QNetworkRequest request = createRequest(url);
