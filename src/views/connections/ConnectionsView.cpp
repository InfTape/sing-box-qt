...
void ConnectionsView::setupUI() {
  ...
  m_tableWidget->setHorizontalHeaderLabels({tr("Source"), tr("Destination"),
                                            tr("Network"), tr("Rule"),
  ...
  connect(m_closeSelectedBtn, &QPushButton::clicked, this,
          &ConnectionsView::onCloseSelected);
  connect(m_closeAllBtn, &QPushButton::clicked, this,
          &ConnectionsView::onCloseAll);
...
void ConnectionsView::setProxyService(ProxyService* service) {
  ...
  if (m_proxyService) {
    connect(
        m_proxyService, &ProxyService::connectionsReceived, this,
          ...
          for (int i = 0; i < conns.count(); ++i) {
            ...
            setCell(i, 4,
                    QString::number(conn["upload"].toVariant().toLongLong() /
                        ...
                        " KB");
            setCell(i, 5,
                    QString::number(conn["download"].toVariant().toLongLong() /
                                    1024) +
                        " KB");
