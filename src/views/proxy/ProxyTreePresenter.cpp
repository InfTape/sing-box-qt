...
ProxyTreePresenter::ProxyTreePresenter(QTreeWidget* treeWidget)
...
QJsonObject ProxyTreePresenter::render(
  ...
  while (*oldIt) {
    if ((*oldIt)->isExpanded()) {
      ...
      expandedGroups.insert(groupName.isEmpty() ? (*oldIt)->text(0)
                                                : groupName);
    }
    if ((*oldIt)->isSelected()) {
  ...
  for (auto it = cachedProxies.begin(); it != cachedProxies.end(); ++it) {
    ...
    auto* countLabel =
        new QLabel(asCountText(formatNodeCount, all.size()), groupCard);
    countLabel->setObjectName("ProxyGroupMeta");
    auto* currentLabel = new QLabel(
        now.isEmpty() ? QString() : asCurrentText(formatCurrent, now),
        groupCard);
    ...
    QObject::connect(
        toggleBtn, &ChevronToggle::toggled, [groupItem](bool expanded) {
...
void ProxyTreePresenter::updateSelectedProxy(
    QJsonObject&            cachedProxies,
    const QString&          group,
    const QString&          proxy,
    const CurrentFormatter& formatCurrent) const {
